...
int getTheNumber(int arr1[][MAX_SIZE], int arr2[][MAX_SIZE], int row, int col, int commonDimension) {
    int result = 0;
...
void multipliedArrays(int arrMultiplied[][MAX_SIZE], int arr1[][MAX_SIZE], int arr1_row, int arr1_col,
    ...
    for (int i = 0;i < arr1_row;i++) {
        for (int j = 0; j < arr2_col;j++) {
            arrMultiplied[i][j] = getTheNumber(arr1, arr2, i , j, arr1_col);
        }
    }
}
...
void run() {
    ...
    if (arr1_col == arr2_row) {
        int arrMultiplied[MAX_SIZE][MAX_SIZE];
