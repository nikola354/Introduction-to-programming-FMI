...
**Задача 10:** Напишете функция `calculateDeterminant(const int mtx[][], size_t rows, size_t cols)`, която изчислява детерминанта на квадратна матрица с размери **N x N**.
...
**Задача 14:** Да се напише функция, която връща най-късия път от точка A(x, y) до точка B(x, y) на кон на шахматна дъска с размер **N x N** (N < 8).
